...
\section{6- Graficación del Modelo y de Instancias de los Algoritmos}

\subsection{Modelo}
Se requiere una gráfica de las **curvas de nivel (Contour Plot)** de la función $f(x,y)$ en la región de interés (alrededor del óptimo, que se espera esté en $x \approx -1$, $y=0$).
\begin{center}
    
\end{center}
La gráfica mostrará contornos elípticos o ligeramente distorsionados, característicos de una función convexa.

\subsection{Instancias de los Algoritmos}
Sobre la gráfica de contorno, se superpondrán las **trayectorias** $\mathbf{x}_0 \to \mathbf{x}_1 \to \dots \to \mathbf{x}^*$ seguidas por el GD y el NAG para los diferentes puntos de partida.
\begin{itemize}
    \item \textbf{GD:} La trayectoria será zigzagueante si el problema está mal condicionado, o más directa si está bien condicionado, acercándose al mínimo.
    \item \textbf{NAG:} Se esperará una trayectoria más suave y con menos oscilaciones que el GD, demostrando la naturaleza acelerada del algoritmo.
\end{itemize}
