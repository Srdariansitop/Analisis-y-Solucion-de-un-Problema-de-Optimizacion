...
donde:
 ...
 \textbf{Eficiencia Computacional}: Evita el cálculo explícito del Hessiano, reduciendo significativamente el costo computacional.\textbf{Convergencia Superlineal}: Bajo condiciones adecuadas, exhibe convergencia superlineal, más rápida que los métodos de gradiente.\textbf{Preservación de Definida Positividad}: La actualización BFGS mantiene la definida positividad de $H_k$ si se satisface la condición de curvatura $y_k^T s_k > 0$.\textbf{Robustez}: Funciona bien en una amplia variedad de problemas sin requerir ajustes finos de parámetros.\textbf{Escalabilidad}: Es adecuado para problemas de mediana dimensión (decenas a cientos de variables).


El algoritmo implementado consta de tres componentes principales: \textbf{Función Objetivo y Gradiente}: Define el problema de optimización a resolver.\textbf{Búsqueda de Línea con Armijo}: Garantiza un decrecimiento suficiente en cada iteración mediante la condición:
    \begin{equation}
    f(x_k + \alpha p_k) \leq f(x_k) + c \alpha \nabla f(x_k)^T p_k
    \end{equation}\textbf{Actualización BFGS}: Mejora iterativamente la aproximación del Hessiano inverso.

En la implementación mostrada, se incluyen varias salvaguardas numéricas:
\textbf{Condición de Curvatura}: Se verifica $y_k^T s_k > 10^{-10}$ antes de actualizar $H_k$
\textbf{Criterio de Parada}: Basado en la norma del gradiente $\|\nabla f(x_k)\| < \text{tol}$
\textbf{Límite de Iteraciones}: Evita ciclos infinitos con un máximo de iteraciones
\textbf{Estabilización Numérica}: Uso de log-sum-exp para evitar overflow numérico.
